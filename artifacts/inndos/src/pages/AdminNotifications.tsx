...
import {
  ...
  RotateCcw, Save, ChevronRight, BellRing, Tag, X, Search as SearchIcon,
} from "lucide-react";
...
// ── Panel Component (used inside Dashboard) ────────────────────────────────────

export function NotificationTemplatesPanel({ token }: { token: string | null }) {
  ...
  function insertVariable(varName: string) {
    ...
    if (ta) {
      ...
      const next  = editBody.slice(0, start) + tok + editBody.slice(end);
      setEditBody(next);
      setTimeout(() => {
        ta.focus();
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Edit SMS, email, and bell message content — changes apply to all future notifications</p>
        </div>
        <Badge variant="outline" className="text-xs">{templates.length} templates</Badge>
      </div>

      {/* Panel body */}
      <div className="flex flex-1 min-h-0 border rounded-xl overflow-hidden bg-white shadow-sm">

        {/* Left panel — filters + list */}
        <div className={`flex flex-col border-r bg-white ${selected ? "hidden lg:flex lg:w-80 lg:flex-shrink-0" : "flex flex-1 lg:flex-none lg:w-80 lg:flex-shrink-0"}`}>
...
// ── Default export kept for the App.tsx route (redirects into Dashboard) ───────
export default function AdminNotifications() {
  const token = localStorage.getItem("token");
  // Redirect to the dashboard notifications tab
  window.location.replace("/#/dashboard?tab=notif-templates");
  return null;
}
