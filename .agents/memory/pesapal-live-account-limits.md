...
PesaPal can also return an IIS HTML HTTP 500 from `SubmitOrderRequest` after authentication and IPN resolution both succeed. Sanitize the HTML before showing it to customers and do not retry automatically because an ambiguous gateway failure could create duplicate orders.